...
void main(){
    ...
    vec4 currentField=get(field,0.,0.);
    ...
    if(pointer.x!=.5){
        ...
        if(distancePointerPixel<tap){
            ...
            //bright
            //sumR=mix(abs(cos(time/2.)+0.5) ,sumR,.3);
            //sumG=mix(abs(cos(time/2.)+0.5),sumG,.5);
            //sumB=mix(abs(cos(time/2.)+0.5),sumB,.1);
            
            //bright color and good colors
            //sumR=mix(abs(cos(time/2.5))+0.5,sumR,.3);
            //sumG=mix(abs(cos(time/2.))+0.5,sumG,.5);
